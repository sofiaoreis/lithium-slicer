...
(Accessed \today)}. Our implementation is publicly available at 
\url{https://github.com/damorim/lithium-slicer}.
...
\begin{table}[h]
  \small
	...
	\begin{tabular}{lrr}
		...
		\bottomrule
	\end{tabular}
	\caption {\combpar{k} performance on capturing faulty statements, as
...
on the cases where \combpar{k} outperformed the baseline technique in at least one $k$.
...
those cases \combpar{k} seems very beneficial.
...
average) statements. For $k=10$, \combpar{k} achieves a mean
...
performance of each pair of techniques is different (except for \combpar{5} 
versus \combpar{10}, where there is no significance).
...
\combpar{k}, improves the diagnostic accuracy up to $73.7\%$ ($13.4\%$
on average).
...
e a Tecnologia (FCT), with the reference PTDC/CCI-COM/29300/2017. The work
reported in this article was supported by national funds through Fundação
para a Ciência e a Tecnologia (FCT) with reference UID/CEC/50021/2019.
...
%%  LocalWords:  SFL dataset suspiciousness MBSD DS DX anonymized NPE
%%  LocalWords:  ij pq Ochiai ochiai significative Siegel hoc amongst
%%  LocalWords:  toolset scalability
