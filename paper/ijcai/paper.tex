...
automate debugging. Dynamic Slicing~\cite{Agrawal:1990:DPS:93542.93576} looks for the statements
...
exception. In contrast to \ds{}, \sfl{}~\cite{7390282} does not analyze
code. It is a black-box~
...
today.\Mar{$\leftarrow$ revise. inconsistent. there is a recent paper
  building on that idea...from 2018.} Several reasons could justify that observation. One hypothesis is that results reported in prior work are
...
\caption {Characterization of \dfj{} subjects.\Mar{numbers seem too
    ...
    *k*LOC or just LOC? please, confirm.}}
\label{tab:df4j}
\end{table}
\normalsize
...
$k\in\{5,10\}$. \Mar{Simplified and moved this par. from Section
  ...
  below. Please, explain and provide rationale}.
...
using\Mar{$\leftarrow$Sofia, I found this par. confusing.}

\begin{equation}
    \Delta{}C = C(\textrm{SFL}) - C(\textrm{SFL+DS-k})
\end{equation}

where C is the cost of diagnosis. \Sof{Need to explain this better}
...
\subsubsection{RQ1: \textit{Diagnosis capability}}
...
\begin{table*}[h]
	\centering
	  \begin{tabular}{|l|ccc|cccc|c|}
		...
		\chart{}           & 84.6\%  & 92.3\% & 7.7\%  & 20 & 76.9\% & 22 & 84.62\% & 26    \\  %\comments{& 92    &  -}\\
		% \jtime{}           & 77.78 \%  & 100.00 \% & 22.22 \%       \\   %\comments{& 76    &  -}\\
		% \mockito{}         & 63.16 \%  & 71.05 \% & 7.89 \%    \\     %\comments{& 118   &  -}\\
		\bottomrule
	\end{tabular}
...
\subsubsection{RQ2: \textit{How often DS misses faulty statements?}}

In \textbf{RQ2}, we evaluate how often DS misses faulty statements, since it is one of the main problems of dynamic slicing techniques. 
...
\Sof{@todo: Answer the question} 

\subsubsection{RQ3: \textit{How effective is the \comb{} combination for bug localization?}}


In \textbf{RQ3}, we intend to evaluate the impact of combining \ds{} to improve \ds{}.
...
\begin{table}[h]
	...
	\begin{tabular}{ccc}
		...
		\bottomrule
	\end{tabular}
	\caption {\Sof{This can be a barchart} Diagnosis Performance for using \comb{}-5 and \comb{}-10 instead of \sfl{}}
	\label{tab:diagnosis}
\end{table}
\normalsize
...
Shapiro-Wilk & \makecell{W = 0.40 \\ p-value = 2.95 x $10^{-9}$} & \makecell{W = 0.48 \\ p-value = 1.54 x $10^{-8}$} & \makecell{W = 0.46 \\ p-value = 9.11 x $10^{-9}$}  \\ \midrule
Friedman & \multicolumn{3}{c}{\makecell{$\chi^{2}$ = 10.46 \\ p-value = 2.95 x $10^{-9}$}} \\ 
\bottomrule
\end{tabular}
  \caption {Statistical tests \Sof{Only for \chart{} yet}}
  \label{table:st}
\end{table*}

\begin{figure}[h]
		\includegraphics[width=0.45\textwidth]{figures/heatmap_nemenyi_result.pdf}
		\caption{Nemenyi post-hoc analysis results \Sof{Only for \chart{} yet}}
