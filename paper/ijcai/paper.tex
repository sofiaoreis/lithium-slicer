...
evaluate fault localization research. The contributions of this work
are: 1)~an empirical study, using real-world applications and bugs, on
the combination of \ds{} and \sfl{} for bug localization of
\emph{Java} faulty programs and 2)~a tool that implements the
combination of \ds{} with \sfl{}, coined \comb{}.  The tool as well as
a replication package will be available once double-blind requirements
are lifted.
...
The first question \Fix{...elaborate...}
The second question addresses the problem that affects all dynamic
slicing techniques~\cite{Lin:2018:BDE:3238147.3238163}---that of
missing faulty statements (see
Section~\ref{sec:slicing}). Understanding that issue is important; the
consequence for debugging is that developers would never be able to
locate faults. The third research question evaluates the impact of the
combination. To sum, our results show that the combination of the two
approaches improves the accuracy of fault localization on average by
\avgImprov\ in 51\% of the studied faults.
...
Originally, this benchmark includes six different subjects and 395 faults (Table
...
the faults of this dataset are FOOs. \Sof{@todo: Explain better what are FOOs}
...
reproducible in the machine used to run the experiments. \textbf{RQ1} evaluates how effective is \ds{} on reducing the test cases, \textbf{RQ2} reviews
...
In~\ref{table:fsws}, we observe that 


\combpar{k}
finds the faulty statements faster than \sfl{}$^{k}$. For both $k$ scenarios ($k=5$ or $k=10$), \combpar{k} performs better, i.e., it misses less faulty statements than \sfl{}$^{k}$.



\combpar{k}The best scena $23$ of the $260$ ($8.8\%$) faults do not include the
faulty statements in the slicer report after slicing the $10$ highest faulty classes.


Using \combpar{10} we are able to find more $20$ faulty
statements than when only using \sfl{}$^{10}$. \combpar{10} only misses $11.8\%$
of the faulty statements whereas \sfl{}$^{10}$ misses $17.9\%$ of the faulty
statements. Thus, \combpar{10} misses less $5.2\%$ of the faults against
\sfl{}$^{10}$.
...
techniques, \sfl{}$^{k}$ and \combpar{k} for $K=5$ and $k=10$. We observe that
$33$ of the $260$
...
statements where removed using \combpar{k}. Inside parentheses, we present the
...
using \combpar{5} and \combpar{10} against \sfl{}$^{5}$ and \sfl{}$^{10}$,
...
the distributions are very similar. \Rui{so??}
