...
the diagnostic accuracy up to $73.7\%$ (\avgImprov\ on average)\Sof{Calculei
mal a média antes. A melhoria média para o top-5 é 9.6\% e a máxima
melhoria que a técnica atingiu foi de 96.7\%. Eu reportei aqui no abstract para o
top-10 porque a \% de melhoria é maior. Se vocês acharem que é mais vantajoso
reportar os valores do top-10, avisem. Podemos reportar ambas. Atenção, só considero
os casos em que pelo menos uma das variantes obteve melhoria}. To sum,
...
\Sof{A primeira citação estava repetida. @Rui, por favor, verifica se a segunda é a que eu introduzi
mayer2008prioritising ou se é outra ou então se estava repetida por engano.}
...
purpose~\Fix{cite}\Sof{Not sure what to cite here}. A fault of omission occurs when
the program is buggy because some code is missing, i.e., a bug that is 
corrected by adding code. Intuitivelly, the ability of techniques to
produce good diagnosis in those cases is poor since fault of omission
are non-executable statements which means that the execution information
cannot help locate the fault directly. A total of $59.7\%$ of the cases 
in this dataset manifest fault of omission, which is a good indication of 
difficulty\Sof{note to self: Improve the good indication of difficulty}.
...
is slightly improved for $91.2\%$ if we consider 
the top $10$ of the highest-ranked faulty classes.
\Sof{Percebo o objectivo da tabela 3 mas não seria melhor arranjar 
maneira de não precisar de repetir? Estes resultados estão na tabela 4}
...
The first experiment we ran to assess impact consists of measuring the
\emph{difference of diagnosis cost}. This metric has been previously
used in other studies~\Fix{cite cite}. More specifically, we computed
...
\begin{figure}[h]
		\centering
		\includegraphics[width=0.42\textwidth]{figures/boxplot_>0.pdf}
		\caption{Boxplot of $\Delta{}C$. \Sof{Considering only the best cases}}
		\label{fig:boxplot}
\end{figure}

\begin{figure}[h]
		\centering
		\includegraphics[width=0.42\textwidth]{figures/boxplot_all.pdf}
		\caption{Boxplot of $\Delta{}C$. \Sof{Considering the best cases and the \textbf{top-misses}}}
		\label{fig:boxplot}
\end{figure}

If we focus at the $116$\Mar{117?} faults where there was improvement
in at least one of the variants, the mean improvement in the ranking
was 41.1 statements for $k=5$ and 18.7 statements for $k=10$. We
observed an impressive maximum ranking improvement of $3170.5$\Mar{why
  the .5 if this is the maximum case?} statements for $k=5$ and $209$
statements for $k=10$. Such difference is justified by the fact that
...
These $227$ faults contain cases where the faulty class was not in the choosen set of classes $k$ and a high amount of cases where the improvement was none. 
...
pairs of techniques. Each square\Mar{gray intensity?} represents the
significance\Mar{of what?} between the corresponding techniques, i.e.,
...
project since it requires high CPU to perform the \combpar{k} evaluation.
