...
faulty classes and that \combpar{10} is the variant of our approach
...
statements. $\Delta C >0$ means that \combpar{k} outperformed the
baseline, \ie{} the mean position of the faulty statements is smaller
in \combpar{k} compared to that of the baseline. $\Delta C <0$ means
that \combpar{k} performs worse compared to its baseline. That could
happen if dynamic slicing misses the faulty statement. $\Delta C=0$
means that the faulty statement remained in the same position.  It is
worth noting that we assume perfect bug
...
relative to \sfl{}$^{k}$. The figure shows a histogram over
$\Delta{}C$ for different values of $k$. Note that (1) the number of
cases where the baseline outperforms \combpar{k} is small (\eg{}, $33$
of the \numFaults\ faults for $k=10$) and that (2) the number of cases
\combpar{k} outperforms the baseline is significative (\eg{}, $117$ of
the \numFaults\ faults for $k=10$). One of the following reasons
justifies the baseline outperforming \combpar{k}:\Mar{following
  sentence is confusing IMO. suggest to clarify or remove. aren't
  points two and three equivalent? as for one, isn't foo an issue both
  for \combpar{k} and \sfl{}$^{k}$?} (1) a fault of omission, (2) the
test has design limitations which \ds~is not capable of handling, or
(3) the buggy-class in not in the top $k$ of the highest-ranked faulty
...
Figure~\ref{fig:boxplot} shows the distributions of $\Delta{}C$. On
the left-hand-side, the figure shows the distributions of $\Delta{}C$
for different values of $k$, considering all faults except those where
test engineering problems and faults of omission are manifested ($227$
faults). The two negative outliers present the cases where the
buggy-class was not in the top $k$. On the right, we only consider the
cases where at least one of the variants resulted in an increase of
$\Delta{}C$ ($116$ faults), i.e., the cases where \ds{} improves
\sfl. Although, we observe a considerable amount of cases where
$\Delta{}C$ is negative or zero in Figure~\ref{fig:diagnosis}, we also
see that there is a considerable amount of faults which could benefit
a lot with \combpar{k}, in Figure~\ref{fig:boxplot}.
...
\begin{figure}[h]
        ...
        distribution considering all cases where \combpar{k}
