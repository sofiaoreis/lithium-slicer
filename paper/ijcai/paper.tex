...
in code that influence some point of interest, such as the evaluation
...
statements. It produces on output a list of program entities ranked in
...
\numFaults{} faults from \numPrograms{} programs from the Defects4J
(D4J) dataset~\cite{just-defects4j-issta2014}, which is frequently
used to evaluated fault localization research. The intuition for the
...
but they used non-representative set of subjects in their evaluation
or used less rigorours methods to assess improvement of techniques compared to those used
...
our results show that \Sof{@todo: sum up the results}.
...
Critical Slicing~\cite{DeMillo:1996:CSS:229000.226310} for its
...
slices that are executable.  Critical Slicing simplifies the original
...
tool~\cite{lithium}. Lithium implements the DD-min Delta Debugging
...
such that the test produces the same failure manifestation. The
Lithium minimization process starts by determining the initial size of
...
elsewhere~\cite{lithium-complexity}. Our implementation is publicly
available.
...
\section{The \sfl{}+Critical Slicing combination}
\label{sec:combination}

To illustrate the \sfl{}+slicing combination, consider a debugging
scenario with five components $c_{1..5}$ and five transactions
$t_{1..5}$, two of which are failing. Consider additionally that the
component $c_2$ is faulty.  Figure~\ref{fig:illustration} shows, at
the left-hand side of the arrow ($\Rightarrow$), the spectra for this
scenario and its corresponding ranking, produced with the Ochiai
predictor. Each line in the ranking shows the ranking position, the
label of the component, and the Ochiai score (in parentheses). The
spectra and ranking appearing at the right-side of the arrow are
obtained with the \sfl{}+slicing combination, assuming that the slice
for the transaction $t_2$ is $\{c_2, c_5\}$.  Intuitively, slicing
enables identification of cells in the spectra whose values can be set
to zero, \ie{}, the components marked with 0 are irrelevant to
determine the test output. Let us now observe the impact of this
change at the ranking. Analyzing the Ochiai formula, one can observe
that, for the statements $j$ which are not part of the slice of a
failing test, the combination reduces the value of $\N{11}$ and
increases the value of $\N{01}$. Therefore, suspiciosness of those
statements decrease. Similar argument applies to other fault
predictors. In the example from Figure~\ref{fig:illustration},
components $c_3$ and $c_4$, which are not in the slice of $t_2$, have
their suspiciousness reduced, enabling the faulty component $c_2$ to
rise from the third to the first position in the ranking.
...
\Sof{Maybe discuss the limitations of \sfl{} based on the capabilibility of \ds{}}The combinination of critical slicing and \sfl{} consists of the
...
\lang{} is a library that provides a set of helper utilities for the
     ...
     APIs. \mockito{} is a mocking testing framework.\comments{ that
       enables the developer to simulate the behavior of classes that
       cannot be used in unit testing.}
...
\begin{table}[h]
  ...
  \setlength{\tabcolsep}{4pt}
    \begin{tabular}{lrrr}
      \toprule
      Project            & Size (kLOC) & \# Tests & \# Faults \\ %\comments{& Failing Test Cases &}
      \midrule
      ...
      \bottomrule
  \end{tabular}
\caption {Characterization of \dfj{} subjects.\Mar{numbers seem too
    big. is this really in
    *k*LOC or just LOC? please, confirm.}\Sof{I think it is a mistake and the unit should be LOC. But I will check.}}
\label{tab:df4j}
\end{table}
\normalsize
...
The evaluation techniques are SFL and \comb{}-$k$, for
$k\in\{5,10\}$. \Mar{Simplified and moved this par. from Section
  4.0. I noticed you used this terminology (\comb{} with ``-k'')
  below. Please, explain and provide rationale}.\Sof{Not sure if it is the right one, an alterantive is using only SFL+DS
  and instead of top-5 and top-10 use K = 5 and K = 10... Which do you think is the best?}
...
using\Sof{@todo: revise it, it is confusing}

\begin{equation}
    \Delta{}C = C(\textrm{SFL}) - C(\textrm{SFL+DS-k})
\end{equation}

where C is the cost of diagnosis. \Sof{@todo: Need to explain this better}
...
\begin{table*}[h]
	\centering
	  \begin{tabular}{|l|ccc|cccc|c|}
		...
		\midrule
		% \lang{}            & 84.60 \% & 84.60 \% & 0 \%       \\   %\commentst{& 124   &  -}\\
		...
		\bottomrule
	\end{tabular}
  \caption {Number of faults where at least one of the buggy-lines is in the SFL and SFL+DS-$k$ report \Sof{@todo: Add absolute numbers for SFL}}
  \label{table:fsws}
\end{table*}  
...
project. Table \ref{table:performance}, reports the average rank of all faulty statements for each version of \chart{} project after
...
\begin{table}[h]
	...
	\begin{tabular}{ccc}
		...
		\bottomrule
	\end{tabular}
	\caption {Diagnosis Performance for using \comb{}-5 and \comb{}-10 instead of \sfl{} \Sof{Considering \chart{}, \jtime{}, \mockito{}}}
	\label{tab:diagnosis}
\end{table}
\normalsize
...
Shapiro-Wilk & \makecell{W = 0.56 \\ p-value = 4.00 x $10^{-15}$} & \makecell{W = 0.69 \\ p-value = 1.81 x $10^{-12}$} & \makecell{W = 0.65 \\ p-value = 1.68 x $10^{-13}$}  \\ \midrule
Friedman & \multicolumn{3}{c}{\makecell{$\chi^{2}$ = 7.22 \\ p-value = 0.027}} \\ 
\bottomrule
\end{tabular}
  \caption {Statistical tests \Sof{Considering \chart{}, \jtime{}, \mockito{}}}
  \label{table:st}
\end{table*}
...
\begin{figure}[h]
		...
		\caption{Nemenyi post-hoc analysis results \Sof{Considering \chart{}, \jtime{}, \mockito{}}}
...
\Sof{@todo} Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
...
\Sof{maybe} Lorem Ipsum is simply dummy text of the printing and typesetting industry.Lorem Ipsum is simply dummy text of the printing and typesetting industry.
