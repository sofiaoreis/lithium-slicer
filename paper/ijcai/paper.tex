...
The \sfl{}+\cs{} combination consists of four steps:
\begin{enumerate}
  \itemsep0em 
  \item\label{step:spectra} Compute spectra $S$ and ranking $R$ for the input test suite;
  \item Select top $k$ most suspicious source files, according to $R$;
  \item\label{step:compute-slice} Compute slices for every $k$ and every failing test;
  \item Adjust the spectra $S$, obtained in Step~\ref{step:spectra}, with
    the results obtained in Step~\ref{step:compute-slice} and recompute ranking;
\end{enumerate}
...
To illustrate the \sfl{}+\cs{} combination, consider a debugging
...
obtained with the \sfl{}+\cs{} combination, assuming that the slice
...
that, for the components $j$ which are not part of the slice of a
failing test, the combination reduces the value of $\N{11}$ and
increases the value of $\N{01}$. Therefore, suspiciosness of those
components decrease. Similar argument applies to other fault
