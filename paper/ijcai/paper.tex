...
the code for every failing tests. More precisely, the \sfl{}+\cs{} combination consists of four steps:
\begin{enumerate}
  \itemsep0em 
  \item\label{step:spectra} Compute spectra $S$ and ranking $R$ for the input test suite;
  \item\label{step:susp-files} Select top $k$ most suspicious source files, according to $R$;
  \item\label{step:compute-slice} Compute slice for every suspicious
...
suspiciosness of those components decrease. Similar argument applies
