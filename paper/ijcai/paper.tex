...
\begin{abstract}
  Software debugging is difficult. One of its main challenges is the improvement of Spectrum-based Fault Localization (SBFL) scores accuracy. We suspect that several highly ranked statements - covered by failing executions - may be unrelated to the fault. This paper reports the results of a comprehensive study perfomed to reassess the impact of combining Dynamic Slicing with SBFL to improve Software Fault Localization, a technique we coin as Tandem-FL. We leverage the Mozilla's Lithium engine to minimize the top-5 and top-10 classes that comprise the highest scored faulty statements of the Defects4j failing tests. The SFL score is computed before and after applying DS to calculate the impact of Tandem-FL.
  Our results show that adding DS to SFL improves its diagnosis accuracy. 
\end{abstract}
...
The output is extracted from the stacktrace obtained through the failing test execution. The stacktrace provides information not only about the place where the fault was revelead but also the path taken to get there. Thus, our approach considers not only the exception caught by the failling test but also the path that was taken to reveal the fault. An example is provided in the following Box:
