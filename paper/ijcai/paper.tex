...
\begin{figure}[h]
		...
		the cases where \combpar{k} achieved improvement under one of the variants}
		\label{fig:boxplot}
		\vspace{-0.2cm}
...
cases that are result of the limitations mentioned before (test design and fault of omission). These results show that \ds{} improves \sfl{} in a mean
of $6$ ($3.3\%$ on average) statements on the average ranking of all faulty statements, when considering $k=10$. Inside parentheses, we present the
rank results for all faults which do not report promissing results. However, If we only consider the faults which can benefit from our approach, we can observe that there is a considerable
...
and Spectrum-based Fault Localization (\sfl) are very popular fault diagnosis techniques and normally seen as complementary. \ds{} is This paper reports the outcome of
