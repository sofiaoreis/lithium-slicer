...
\begin{abstract}
Several approaches have been proposed to improve automated softaware fault
diagnosis, aiming to reduce the effort and costs spent in debugging. This
paper reports the outcome of a study
performed to reassess\Rui{dismistify? We need to say something on why we
decided to mistify this combination...} the impact of combining Dynamic Slicing (DS) with
spectrum-based fault localization (SFL), a technique we coin as Tandem-FL. We
suspect that several highly suspiscious components \Rui{I am using components to be
more generic} --- often involved in failing, but seldom in passed executions ---
may be unrelated to the bug. Thus, we use Mozilla's Lithium engine to minimize
the failing tests source files that comprise the highest scored faulty
statements from the Defects4j dataset\Rui{not clear why this is in here for
the reader: Mozilla, minimizing test cases, ... }. The impact of Tandem-FL is evaluated
through computing the SFL score \Rui{SFL score: a reader does not know what that is}
of each fault before and after minimization\Rui{minimization: unknown to the
reader... He thinks that the study is on slicing}.
This study also discusses on limitations of dynamic slicing, such as the potential
of missing faulty statements. Our
results show that adding combining SFL with dynamic slicing improves the diagnostic
accuracy in \Rui{x\% on average}, hence leading to the root cause of oberved failures faster.
\end{abstract}
...
of a failing assertion. In contrast to \ds{} \Rui{DS, not introduced at this point
in the intro}, \sfl{}~\cite{7390282}
...
This paper reports the results of a comprehensive study evaluating the impact of
combining \ds{} and \sfl{} to improve software fault localization, a technique
we coin as \comb{}. The intuition for the combination is that several highly
ranked statements, albeit covered by failing executions, may be unrelated to the
fault. Our empirical study involves \numFaults{} faults from \numPrograms{}
different programs comprised in the Defects4J (D4J)
dataset~\cite{just-defects4j-issta2014}, which is frequently used to evaluate
fault localization research. Prior work reported promising results in this
...
\sfl{} with \ds{}; it also discusses the oracle problem.

\subsection{Combining \sfl{} and \ds{}}
\label{sec:comb}
...
\begin{table*}[h]
	...
	\centering
	  \begin{tabular}{l|cccc|cccc|c}
		\toprule
		\multirow{2}{*}{Project}            & \multicolumn{4}{c|}{SFL}  & \multicolumn{4}{c|}{\combpar{k}} & \multirow{2}{*}{\# Faults} \\

		            & \# $k = 5$ & \% $k = 5$ & \# $k = 10$ & \% $k = 10$
					& \# $k = 5$ & \% $k = 5$ & \# $k = 10$ & \% $k = 10$ &  \\
		\midrule
		 \lang{}         &  54 & 84.4\%  & 54  & 84.4\%
		 				& 64 & 100\% & 62 & 96.9\% & 64      \\
		\cmath{}           & 82 & 81.2\%  & 86 & 85.1\%
						& 90 & 89.1\% & 97 & 96.0\% & 101  \\   %\comments{& 177   &  -}\\
		...
		\bottomrule
	\end{tabular}
  \caption {Number of faults where at least one of the buggy-lines is in the SFL and \combpar{k} report \Rui{explain what $k$ is for SFL alone.}}
  \label{table:fsws}
\end{table*}
...
Mean & 522.47     & 684.89   & 611.04   \\ \midrule
Median & 50.375      & 66.25               & 54.3                \\ \midrule
Variance & 1191.12      & 1361.83   & 1301.78    \\ \midrule
Shapiro-Wilk & \makecell{W = 0.48 \\ p-value = 1.10 x $10^{-26}$} & \makecell{W = 0.55 \\ p-value = 4.31 x $10^{-25}$} & \makecell{W = 0.52 \\ p-value = 6.23 x $10^{-26}$}  \\ \midrule
Friedman & \multicolumn{3}{c}{\makecell{$\chi^{2}$ = 28.97 \\ p-value = 5.13 x $10^{-7}$}} \\
\bottomrule
\end{tabular}
...
\begin{figure}[h]
		...
		\caption{Nemenyi post-hoc analysis results \Sof{@tofix: Need to center ylabels}}
