...
\begin{abstract}
Software debugging is difficult. Several approaches have been proposed to the improvement of Spectrum-based Fault Localization (SBFL) scores accuracy. This paper reports the outcome of a study performed to reassess the impact of combining Dynamic Slicing (DS) with SBFL to improve Software Fault Localization, a technique we coin as Tandem-FL. We suspect that several highly ranked statements - covered by failing executions - may be unrelated to the fault. Thus, we use Mozilla's Lithium engine to minimize the failing tests source files that comprise the highest scored faulty statements from the Defects4j dataset. The impact of Tandem-FL is evaluated through computing the SFL score of each fault before and after minimization. This study also reflects on the DS limitation of missing faulty statements. Our results show that adding DS to SFL improves the SFL diagnosis accuracy.\Sof{add numbers tomorrow morning after closing results}
\end{abstract}
