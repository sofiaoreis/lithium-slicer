...
cases where the baseline outperforms \combpar{k} is small (\eg{},
\numOutPerformed\ of the \numFaults\ faults for $k=10$) and that (2)
the number of cases \combpar{k} outperforms the baseline is
significative (\eg{}, $117$ of the \numFaults\ faults for $k=10$). One
of the following reasons justifies the baseline outperforming
\combpar{k}:\Mar{following sentence is confusing IMO. suggest to
  clarify or remove. aren't points two and three equivalent? as for
  one, isn't foo an issue both for \combpar{k} and \sfl{}$^{k}$?} (1)
a fault of omission, (2) the test has design limitations which \ds~is
not capable of handling, or (3) the buggy-class in not in the top $k$
of the highest-ranked faulty classes under analysis.
...
\combpar{k}. On the left-hand-side, we show distributions of
...
the cases of faults where the techniques perform equally.\Mar{sofia,
  are you sure there are no zeros in this dataset? The min. in this
  distibution (as per the base of the plot) seems to be 0.} To sum,
