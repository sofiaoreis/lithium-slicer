...
of missing faulty statements, which \ds{}'s key limitation, is not
...
(\avgImprov\ on average).\Mar{do we have supporting data to say that
  the improvement is bigger when we care the most? for the cases SFL
  performs worse? if yes, can we show average for that?} To sum, results indicate that the
...
DX community\Mar{I don't know what is DX. suggest to expand the
  acronym and/or cite.}\Mar{say shortly (in general terms) what is the
  goal. what is that good for.}, a sub-field of AI~\cite{REITER198757,DEKLEER200325}. Applications
...
well. \Mar{Sofia, does it make sense to show a histogram of (initial)
...
a considerable amount of cases where $\Delta{}C$ is negative or zero in Figure~\ref{fig:diagnosis}, we also see that there is a considerable amount of faults which could benefit a lot with \combpar{k}, in Figure~\ref{fig:boxplot}.
\begin{figure}[h]
		\vspace{-0.2cm}
		\centering
      ...
      the variants (right)\Mar{por favor, aumente o tamanho dos labels
      (nao consigo enxergar na impressao).}}
		\label{fig:boxplot}
		\vspace{-0.2cm}
...
if we only consider the faults which can benefit from our approach, we can observe that there is a considerable
...
As the results are statistically significant, it is possible to state that \combpar{k} has a positive effect on the localization of some types of bugs. There is indeed a problem on using slicing on fault of omission bugs that needs further research. Although, this dataset has more than $50\%$ of fault of omissions, our
simple slicing approach was capable of improving the ranking of $95$ faults for $k=5$ and $115$ faults for $k=10$. These are promissing results from what Automated
Program Repair techniques may directly benefit to improve the accuracy of their own techniques.
...
project since it requires high CPU to perform the \combpar{k} evaluation. A potential threat to \textit{construct validity} relates to the choice of oracle generation
...
techniques. \Rui{not related work. more of motivation (avoid words such as believe)-->}
...
and Spectrum-based Fault Localization (\sfl) are very popular fault diagnosis techniques and normally seen as complementary. This paper reports the outcome of
