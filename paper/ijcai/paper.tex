...
to reassess the effects of combining \ds{} with \sfl{}.  With this
combination, components that are often involved in failing but seldom
in passing test runs could be located and their suspiciousness
reduced. Results show that the \ds{}-\sfl{} combination, coined as
Tandem-SL, improves the diagnostic accuracy up to $73.7\%$
(\avgImprov\ on average). Furthermore, results indicate that the risk
of missing faulty statements, which is a \ds{}'s key limitation, is not
high---~\ds{} misses faulty statements in $9\%$\comments{($23$
  misses)} of the $\numFaults{}$ cases. To sum, we found that the
\ds{}-\sfl{} combination was practical and effective and encourage new
\sfl{} techniques to be evaluated against that optimization.
\end{abstract}
...
approaches may improve fault localization up to $73.7\%$
(\avgImprov\ on average). To sum, results indicate that the
...
counted the number of cases of ``fault of omission'', a popular metric
in fault localization research for that
purpose~\cite{Pearson:2017:EIF:3097368.3097441}.  A fault of omission
(\foo) occurs when the program is buggy because some code is missing,
i.e., a fix to the bug iss created by adding code as opposed to
modifying or changing code. Intuitively, the ability of techniques to
produce accurate diagnostic reports in those cases might be poor as
techniques can only extract information about existing statements,
which are not problematic. This dataset has a total of $59.7\%$ of
\foo s, making it challenging for techniques to perform
well. 
...
misses the fault is different compared to \combpar{k}. \sfl{}$^{k}$
...
indicating that \combpar{k}, on average, captures more faults compared
to \sfl{}$^{k}$. In only two cases \sfl{}$^{k}$ outperformed
\combpar{k}. These results show that the majority of faulty statements
are found on the $5$ highest ranked faulty classes and that
\combpar{10} is the variant of our approach that performs better
...
with \sfl{} to improve fault localization. The first experiment we ran 
to assess impact consists of measuring the \emph{difference of diagnosis 
cost}. This metric has been previously used in other studies~\cite{7390282,ang-perez-van-deursen-rui-2017,Pearson:2017:EIF:3097368.3097441,Perez:2018:LQR:3304889.3304927}. 
...
\combpar{k}, \ie{}, the combination had a positive impact. $\Delta C
<0$ means that \combpar{k} performs worse compared to its
...
Figure \ref{fig:diagnosis} summarizes performance of \combpar{k}
relative to \sfl{}$^{k}$ using histograms over $\Delta{}C$ for
different values of $k$. Note that (1) the number of cases where
\sfl{}$^{k}$ outperforms \combpar{k} is very small compared and (2)
the number of cases \combpar{k} outperforms \sfl{}$^{k}$ is
...
a considerable amount of cases where $\Delta{}C$ is negative or zero in 
Figure~\ref{fig:diagnosis}, we also see that there is a considerable amount 
of faults which could benefit a lot with \combpar{k}, in Figure~\ref{fig:boxplot}.
...
\begin{figure}[h]
		...
		the cases where \combpar{k} achieved improvement under one of
      the variants (right)}
		\label{fig:boxplot}
		\vspace{-0.2cm}
\end{figure}
...
techniques, \sfl{}$^{k}$ and \combpar{k} for $k=5$ and $k=10$.
In bold, we present the results of the $227$ faults that do not comprise the
cases that are result of the limitations mentioned before (test design and fault
of omission). These results show that \ds{} improves \sfl{} in a mean
of $6$ ($3.3\%$ on average) statements on the average ranking of all faulty statements,
when considering $k=10$. Inside parentheses, we present the
rank results for all faults which do not report promissing results. However,
if we only consider the faults which can benefit from our approach, 
we can observe that there is a considerable
amount of cases where the approach is actually successful. For
$k=5$, our approach achieves a mean improvement of $41.1$ ($9.57\%$ 
on average) statements on the ranking and an impressive maximum ranking improvement
of $3171$ ($96.7\%$ on average) statements. Whereas for $k=10$, \combpar{k}
...
entire files are discarded in \combpar{5}, whose statements are
included in the ranking of \combpar{10}. Consequently, the
...
\toprule
  & SFL$^{5}$  & \combpar{5}       & SFL$^{10}$                 & \combpar{10}              \\ \midrule
...
Shapiro-Wilk &\makecell{$W$ = 0.52 \\ $p$ = 1.75 x $10^{-24}$} & \makecell{$W$ = 0.51 \\ $p$ = 1.10 x $10^{-24}$} & \makecell{$W$ = 0.48 \\ $p$ = 2.50 x $10^{-25}$} &  \makecell{$W$ = 0.48 \\ $p$ = 2.26 x $10^{-25}$}  \\ \midrule
...
distinct. To understand which techniques perform differently, \ie{} to answer
the question \textit{Does \combpar{k} perform differently than
...
statistical significance of the diagnostic accuracy amongst the different 
techniques, i.e., if it is significant it means that the techniques perform
...
As the results are statistically significant, it is possible to state that \combpar{k} 
has a positive effect on the localization of some types of bugs. There is indeed a 
problem on using slicing on fault of omission bugs that needs further research. 
Although, this dataset has more than $50\%$ of fault of omissions, our
simple slicing approach was capable of improving the ranking of $95$ faults for $k=5$ 
and $115$ faults for $k=10$. These are promissing results from what Automated
Program Repair techniques may directly benefit to improve the accuracy of their own 
techniques.
...
project since it requires high CPU to perform the \combpar{k} evaluation. A potential
...
\Rui{this sentence does not belong to rw (We suspect...)-->} We
suspect that several highly suspicious components—often involved in failing, but
seldom in passing test runs—may be unrelated to the bug.
...
\Rui{not related work. more of motivation (avoid words such as believe)-->}
Although, \ds{} does not ensure the improvement of \sfl{} since it
can remove faulty statements from the failing test, we believe it has a great
potential to help reducing test cases and improve \sfl{} accuracy. In this
