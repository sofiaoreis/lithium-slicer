...
main.add_argument("--testname", nargs=1, type=str)
main.add_argument("--classes", nargs=1, type=str)
main.add_argument("--states_path", nargs=1, type=str)
...
states_path = args.states_path[0]
top = args.top[0]
...
# create log_testcase directory - !!@FIX: Needs change
log_testcase_dir = os.path.join(log_dir, testname)
...
logger.info("Running testcase {test_case} from {project}-{bug}".format(test_case=testname, project=project, bug=bug_number))
...
def minimize_file(filepath):
    ...
    """    
    global project, bug_number, states_path
    ...
    copy_tree('source/{}_{}/'.format(project, bug_number), project_dir)
    ...
    if remove_comments:
        ...
        if platform.system() == 'Darwin':
            bash_cmd = '/usr/libexec/java_home -v 1.8 --exec java -cp comments-remover.jar com.tqrg.cleaner.Cleaner ' + java_file + ' ' + uncomment_path
...
#             bash_cmd = '/usr/lib/jvm/java-8-oracle/bin/java -cp java-parser-comments-remover-1.0-SNAPSHOT-jar-with-dependencies.jar com.tqrg.cleaner.Cleaner ' + java_file + ' ' + uncomment_path
    ...
    try:
        ...
        cmd_line = "python3 -m lithium --tempdir={TEMPDIR} interesting {PROJECTDIR} {STATES_PATH} {TESTNAME} {FILE}"
        cmd_line = cmd_line.format(TEMPDIR=lithium_tmp, PROJECTDIR=project_dir, STATES_PATH=states_path, TESTNAME=testname,FILE=java_file)
...
logger.info("The testcase {test_case} was finished in {time} minutes".format(test_case=testname, time=total_time))