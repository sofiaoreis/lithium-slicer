...
if [ ! -f "$ORACLE_PATH" ]; then
    if [ "$DEBUG" == 1 ]; then
	    gen_expected_msg=$(python3 generate_expected_msg.py --project "$PROJECT" --bugnumber "$BUG" --debug | tee /dev/tty) 
	else
    	gen_expected_msg=$(python3 generate_expected_msg.py --project "$PROJECT" --bugnumber "$BUG" | tee /dev/tty) 
    fi
...
gen_inputs=$(python3 generate_inputs.py --project "$PROJECT" --output "$INPUTS" --bugnumber "$BUG" --files_per_bug "$TOP" | tee /dev/tty)
